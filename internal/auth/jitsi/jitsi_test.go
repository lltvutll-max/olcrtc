...
func TestParseRoomURL(t *testing.T) {
	tests := []struct {
		...
		{name: "https url", raw: "https://jitsi.etudevs.ru/" + testRoom, host: "jitsi.etudevs.ru", room: testRoom},
...
func TestProviderIssue(t *testing.T) {
	creds, err := Provider{}.Issue(context.Background(), auth.Config{
		RoomURL: "https://jitsi.etudevs.ru/olcrtc",
		Name:    "olcrtc-test",
	})
	if err != nil {
		t.Fatalf("Issue: %v", err)
	}
	if creds.URL != "jitsi.etudevs.ru" {
		t.Fatalf("URL = %q, want %q", creds.URL, "jitsi.etudevs.ru")
	}
